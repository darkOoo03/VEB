...
Start-Process cmd -ArgumentList "/k dotnet run --project src/Services/AuthService/AuthService.csproj"

Write-Host "Pokretanje TravelService (port 5002)..." -ForegroundColor Yellow
Start-Process cmd -ArgumentList "/k dotnet run --project src/Services/TravelService/TravelService.csproj"

Write-Host "Pokretanje ActivityService (port 5003)..." -ForegroundColor Yellow
Start-Process cmd -ArgumentList "/k dotnet run --project src/Services/ActivityService/ActivityService.csproj"
